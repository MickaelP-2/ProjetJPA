...
//attributs: date, homeTeam, awayTeam, Team, scorer, minute, ownGoal, Penalty
	 ...
	 */
public class CreerScore extends JFrame implements ActionListener,MouseListener{
/**
	 * 
	 */
		...
		if(verifierDonnees()==false) {
			return;
					...
					transaction.begin();//ouverture de la transaction
						...
						GS.setDate(LocalDate.parse(txtf1.getText().toString()));//LocalDate
						GS.setHomeTeam(txtf2.getText().toString());
						GS.setAwayTeam(txtf3.getText().toString());
						GS.setTeam(txtf4.getText().toString());
						GS.setScorer(txtf5.getText().toString());
						GS.setMinute(txtf6.getText().toString());
						GS.setOwnGoal(Boolean.parseBoolean(txtf7.getText().toString()));
						GS.setPenalty(Boolean.parseBoolean(txtf8.getText().toString()));
						em.persist(GS);
