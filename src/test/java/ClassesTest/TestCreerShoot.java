...
public class TestCreerShoot extends JFrame implements ActionListener,MouseListener{
	/**
	 * 
	 */
		...
		if(verifierDonnees()==false) {
			return;
				...
				transaction.begin();//ouverture de la transaction
					TestShootout TSH = new TestShootout();
					TSH.setDate(LocalDate.parse(txtf1.getText().toString()));//LocalDate
					TSH.setHomeTeam(txtf2.getText().toString());
					TSH.setAwayTeam(txtf3.getText().toString());
					TSH.setWinner(txtf4.getText().toString());
					TSH.setFirstShooter(txtf5.getText().toString());
					em.persist(TSH);
				transaction.commit();
				em.close();
		}
	...
	public static void main(String[] args) {
		...
		TestCreerShoot CSH = new TestCreerShoot();
		CSH.setVisible(true);
	}
	@Override
