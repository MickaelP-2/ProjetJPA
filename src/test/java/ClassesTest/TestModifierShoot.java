...
public class TestModifierShoot extends JFrame implements ActionListener,MouseListener{
	/**
	 * 
	 */
	...
	//
	/**
	 * Attributs de la fenetre de modification des tirs au but.
	...
	MessageAide ma;
	FenetreAffichage FA;
	TestShootout SHmodif;//pour une instance unique en vue de la modification
	//
	/**
	 * constructeur de la classe graphique/fenetre.
	 */
	public TestModifierShoot() {
		bl = new BorderLayout();
		...
		JP3 = new JPanel();
		JP4 = new JPanel();
		label = new JLabel("Modification de tirs aux buts.");
	...
	public void rechercheDate() {
		...
		List<TestShootout> list;
		Iterator<TestShootout> iter = null;
		String ligne = null;
		...
		EntityTransaction transaction = em.getTransaction();
			...
			TestShootout SH = new TestShootout();
			if(list.size()>1) { 
				...
				//
				SHmodif.setId(Integer.parseInt(str[0].toString()));
				SHmodif.setDate(LocalDate.parse(str[1].toString()));
				SHmodif.setHomeTeam(str[2].toString());
				SHmodif.setAwayTeam(str[3].toString());
				SHmodif.setWinner(str[4].toString());
				SHmodif.setFirstShooter(str[5].toString());
				//
	...
	public void modifier() {
		System.out.println("modifier shoutouts");
		EntityManagerFactory entityManagerFactory = Persistence.createEntityManagerFactory("config1");//Foot dans persistence.xm
		EntityManager em = entityManagerFactory.createEntityManager();
		EntityTransaction transaction = em.getTransaction();
			transaction.begin();//ouverture de la transaction
			//
			Query query = em.createQuery("UPDATE Shootout sh SET sh.date= :newdate  WHERE sh.date= :olddate");
			query.setParameter("newdate",(LocalDate.parse(txtf1.getText())));
			query.setParameter("olddate",(LocalDate.parse(SHmodif.getDate().toString())));
			query.executeUpdate();
			query = em.createQuery("UPDATE Shootout sh SET sh.homeTeam= :newhomeTeam  WHERE sh.homeTeam= :oldhomeTeam");
			query.setParameter("newhomeTeam",(txtf2.getText()));
			query.setParameter("oldhomeTeam",SHmodif.getHomeTeam().toString());
			query.executeUpdate();
			query = em.createQuery("UPDATE Shootout sh SET sh.awayTeam= :newawayTeam  WHERE sh.awayTeam= :oldawayTeam");
			query.setParameter("newawayTeam",(txtf3.getText()));
			query.setParameter("oldawayTeam",SHmodif.getAwayTeam().toString());
			query.executeUpdate();
			query = em.createQuery("UPDATE Shootout sh SET sh.winner= :newWinner  WHERE sh.winner= :oldWinner");
			query.setParameter("newWinner",(txtf4.getText()));
			query.setParameter("oldWinner",SHmodif.getWinner());
			query.executeUpdate();
			query = em.createQuery("UPDATE Shootout sh SET sh.firstShooter= :newFirstShooter  WHERE sh.firstShooter= :oldFirstShooter");
			query.setParameter("newFirstShooter",(txtf5.getText()));
			query.setParameter("oldFirstShooter",SHmodif.getFirstShooter());
			query.executeUpdate();
			//
			transaction.commit();//validation de la transaction
			em.close();//fermeture du flux
	...
	public static void main(String[] args) {
		...
		TestModifierShoot TMSH = new TestModifierShoot();
		TMSH.setVisible(true);
	}
	@Override
