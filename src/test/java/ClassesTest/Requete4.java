...
/**
	...
	public void requeteMethode4() {
		...
		HashMap<String, TestPourcents> copieFichier;//Composant HashMap types: String pour le nom de l'equipe, Pourcents pour les valeurs
		List<TestResults> list;//type Results
		Iterator<TestResults> iter;//type Results
		...
		EntityTransaction transaction = em.getTransaction();
			...
			copieFichier = new HashMap<String, TestPourcents>();//nomVille, Score(away+home)
			if(list.size()>0) {
				iter = list.iterator();
				...
				while(iter.hasNext()) {//&& i<nb
					...
					if(copieFichier.containsKey(equipe)==true) {
						...
						copieFichier.replace(equipe,new TestPourcents(victoires,defaites,nuls,nbMatch,0.0,0.0,0.0));
						...
						copieFichier.put(equipe,new TestPourcents(victoires,defaites,nuls,nbMatch,0.0,0.0,0.0));
					}
				}
				transaction.commit();
				em.close();
				...
				TestAccueil.txta.setText("Equipe inconnue!!");
			}
			//Lecture de copieFichier
			//declaration d'un objet de type List<Pourcents>
			List<TestPourcents> list2 = new ArrayList<TestPourcents>();
			...
			Iterator<TestPourcents> iterValue = copieFichier.values().iterator();
			//declaration d'un objet de type List<Pourcents>
			List<TestPourcents> list3 = new ArrayList<TestPourcents>();
			TestPourcents pour3 = new TestPourcents();
			String key = null;
			TestPourcents pour = new TestPourcents();//declaration d'une instance de Pourcents
			...
			while(iterKey.hasNext() && iterValue.hasNext()) {
				...
				pour3 = new TestPourcents(key,pour.getVictoires(),pour.getDefaites(),pour.getNuls(),pour.getTotal(),pour.getPourcentageVictoires(),pour.getPourcentageDefaites(),pour.getPourcentageNuls());
				list3.add(pour3);
			}
			//Utiliser la transformation de Pourcents(tri) et l'associer a 
			Collections.sort(list3, TestPourcents.comparateurPourcentageVictoire);//ordre alphabetic des noms
			...
			TestAccueil.txta.setText("Equipe(s) : \n");
			for(int j=0;j<nb;j++) { 
				...
				TestAccueil.txta.append((j+1)+": "+tab2[0].toString()+": "+tab2[5].toString()+" % victoire en "+tab2[4].toString()+" matchs.\n");
			}
	}
	//
	/**
