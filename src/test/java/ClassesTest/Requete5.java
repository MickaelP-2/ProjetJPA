...
/**
	...
	public void requeteMethode5() {
		...
		HashMap<LocalDate, TestPourcents> copieFichier;//Composant HashMap types: String pour le nom de l'equipe, Pourcents pour les valeurs
		...
		EntityTransaction transaction = em.getTransaction();
			...
			TestPourcents PR1=null;
			TestPourcents PR2=null;
			...
			copieFichier = new HashMap<LocalDate, TestPourcents>();//nomVille, Score(away+home)
			if(list.size()>0) {
				iter = list.iterator();
				int i = 1;//pour le nb max a afficher
				while(iter.hasNext()) {//&& i<nb
					PR1 = new TestPourcents();//pourcentages de l'equipe1
					PR2 = new TestPourcents();//pourcentages de l'equipe2
			...
			DecimalFormat format = new DecimalFormat("##0.00");//formatage pour les attributs de type double(2 chiffres apres la virgule)
				PR1 = new TestPourcents(nbVic1,nbDef1,nbNul1,total,0.0,0.0,0.0);
				...
				PR2 = new TestPourcents(nbVic2,nbDef2,nbNul2,total,0.0,0.0,0.0);
				...
				PR1 = new TestPourcents(nbVic1,nbDef1,nbNul1,total,PR1.getPourcentageVictoires(),PR1.getPourcentageDefaites(),PR1.getPourcentageNuls());
				PR2 = new TestPourcents(nbVic2,nbDef2,nbNul2,total,PR2.getPourcentageVictoires(),PR2.getPourcentageDefaites(),PR2.getPourcentageNuls());
			...
			TestAccueil.txta.setText(titre+"\n");
			TestAccueil.txta.append(ligne1+"\n");
			TestAccueil.txta.append(ligne2+"\n");
			TestAccueil.txta.append(ligne3+"\n");
			TestAccueil.txta.append(ligne4+"\n");
	}
	//
	/**
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if(obj == executer) {
			...
			System.out.println("JRB5: "+TestAccueil.JRB5.isSelected());
			requeteMethode5();
		}
		else if(obj == fermer) {
