...
public class TestCreerResultats extends JFrame implements ActionListener,MouseListener{
	/**
	 * 
	 */
	...
	TestResults TRES;//pour importer la classe
	MessageAide ma;
	//
	/**
	 * Constructeur de la fenetre de création de résultat.
	 */
	public TestCreerResultats() {
		//constructeur
		bl = new BorderLayout();
		...
		if(verifierDonnees()==false) {
			return;
					...
					transaction.begin();//ouverture de la transaction
						...
						TRS.setDate(LocalDate.parse(txtf1.getText().toString()));//LocalDate
						TRS.setHomeTeam(txtf2.getText().toString());
						TRS.setAwayTeam(txtf3.getText().toString());
						TRS.setHomeScore(Integer.parseInt(txtf4.getText().toString()));
						TRS.setAwayScore(Integer.parseInt(txtf5.getText().toString()));
						TRS.setTournois(txtf6.getText().toString());
						TRS.setNomVille(txtf7.getText().toString());
						TRS.setCountry(txtf8.getText().toString());
						TRS.setNeutre(Boolean.parseBoolean(txtf9.getText().toString()));
						em.persist(TRS);
	...
	public static void main(String[] args) {
		...
		TestCreerResultats TCR = new TestCreerResultats();
		TCR.setVisible(true);
	}
