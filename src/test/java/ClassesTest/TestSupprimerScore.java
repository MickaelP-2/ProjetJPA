...
//attributs: date, homeTeam, awayTeam, Team, scorer, minute, ownGoal, Penalty
	 ...
	 */
public class TestSupprimerScore extends JFrame implements ActionListener,MouseListener{
/**
	 * 
	 */
	...
	//
	/**
	 * Attributs de la fenetre de suppréssion de scores.
	...
	TestScores SC;//pour importer la classe
	MessageAide ma;
	FenetreAffichage FA;
	//
	/**
	 * Constructeur de la fenetre de suppréssion d'un score de buteur.
	 */
	public TestSupprimerScore() {
		//constructeur
		bl = new BorderLayout();
		...
		JP3 = new JPanel();
		JP4 = new JPanel();
		label = new JLabel("Suppréssion de scores de buteurs.");
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
			TestSupprimerScore SS = new TestSupprimerScore();
			SS.setVisible(true);;
	}
	//
	/**
	...
	public void rechercheDate() {
		...
		List<TestGoalscorers> list;
		Iterator<TestGoalscorers> iter = null;
		String ligne = null;
		...
		EntityTransaction transaction = em.getTransaction();
			...
			TestGoalscorers GS = new TestGoalscorers();
			if(list.size()>1) { 
	...
	/**
	 * Méthode pour implémenter la requete de suppréssion 
	...
	public void supprimer() {
		...
		EntityTransaction transaction = em.getTransaction();
			transaction.begin();//ouverture de la transaction
			Query query = em.createQuery("DELETE FROM Goalscorers gs WHERE gs.id= :id");
			query.setParameter("id",(Integer.parseInt(txtf9.getText().toString())));
			query.executeUpdate();
			transaction.commit();//validation de la transaction
			em.close();//fermeture du flux
